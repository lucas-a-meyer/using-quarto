...
You can then refer to the bibliography by simply adding the appropriate `[@citation-name]` in your text. For example, if my BibTex file contains a `diamond97` and a `diamond11` citation for Jared Diamond's books, I can cite them by writing `[@diamond97]` and `[@diamond11]`. I can then cite _Guns, Germs and Steel [@diamon97] and Collapse [@diamond11]. All [Citation Style Language](https://citationstyles.org) formats are supported.
